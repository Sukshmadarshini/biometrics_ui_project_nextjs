...
/**
 * Registry-controlled institutional TLDs — cannot be freely registered,
 * so MX checks are skipped for these entirely.
 */
const TRUSTED_INSTITUTIONAL_TLDS = [
  ".ac.uk",   // UK universities (e.g. hw.ac.uk, ox.ac.uk)
  ".edu",     // US universities (e.g. mit.edu, stanford.edu)
...
export async function validateEmail(email: string) {
  ...
  /* Known good domains — skip DNS entirely */
  if (KNOWN_GOOD_DOMAINS.has(domain)) {
    const result = { valid: true };
    cache.set(normalized, result);
    return result;
  ...
  /* Trusted institutional TLDs — registry-controlled, skip DNS entirely */
  if (TRUSTED_INSTITUTIONAL_TLDS.some((tld) => domain.endsWith(tld))) {
    const result = { valid: true };
    cache.set(normalized, result);
    return result;
  }
