...
export async function POST(req: NextRequest) {
  try {
    ...
    const emailContents = await getEmailContents(["contact", "returnContact"]);

    const ownerEmailContent = {
      headerTitle:    "New Contact Form Submission",
      headerSubtitle: "Sukshmadarshini™ · Internal Notification",
      bodyText:       "You have received a new message from your website contact form.",
      footerText:     "This email was sent from the Sukshmadarshini™ contact form.",
      ...emailContents["contact"],
    };

    const clientEmailContent = {
      headerTitle:    "We've Received Your Message",
      bodyText:       "Thank you for reaching out to us. We have successfully received your message and our team will get back to you as soon as possible, typically within 1 to 2 business days.",
      nextStepsTitle: "What happens next?",
      step1:          "📬 Your message has been forwarded to our team.",
      step2:          "🕐 We aim to respond within 1 to 2 business days.",
    ...
    const [ownerResult, clientResult] = await Promise.allSettled([
      ...
      resend.emails.send({
        ...
        to:      process.env.EMAIL_TO!,
        subject: `Contact Form: ${subject}`,
        react:   ContactEmail({ name, email, subject, message, emailContent: ownerEmailContent }),
      }),
      ...
      resend.emails.send({
        ...
        to:      email,
        subject: `We received your message: ${subject}`,
        react:   ReturnContactEmail({ name, email, subject, message, emailContent: clientEmailContent }),
      }),
    ]);
