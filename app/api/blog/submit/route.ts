...
export async function POST(req: NextRequest) {
  try {
    ...
    const emailContents = await getEmailContents(["blogOwnerReview", "blogSubmissionReceived"]);

    const ownerEmailContent = {
      headerTitle:    "New Blog Submission",
      headerSubtitle: "Sukshmadarshini Blog Review",
      bodyText:       "A new blog post has been submitted and is awaiting approval.",
      reviewNote:     "Review the full article in Sanity before taking action.",
      approveLabel:   "Approve & Publish",
      rejectLabel:    "Reject",
      footerText:     "Sukshmadarshini™ Blog Management",
      ...emailContents["blogOwnerReview"],
    };

    const authorEmailContent = {
      headerTitle:    "Submission Received",
      headerSubtitle: "Sukshmadarshini Blog Submission",
      bodyText:       "Thank you for submitting your blog post. We have successfully received it and it is currently under review by our editorial team.",
      statusNote:     "Status: Pending Review\nOur team typically reviews submissions within 2 to 3 business days. Please avoid submitting the same article multiple times.",
      nextStep:       "You will receive another email once a decision has been made.",
      signOff:        "Thank you for contributing to Sukshmadarshini.",
      footerText:     "Sukshmadarshini™ Blog Submission System",
      ...emailContents["blogSubmissionReceived"],
    };
      ...
      await resend.emails.send({
        ...
        html: await render(
          BlogSubmissionReceivedEmail({
            ...
            title:    title.trim(),
            readTime,
            emailContent: authorEmailContent,
          })
        ),
      });
    }
